...
export function ResumeUploadPanel({
  ...
  /** Smaller heading and no upload history — used when embedded on the dashboard overview. */
  compact?: boolean;
}) {
  ...
  async function handleParse() {
    ...
    try {
      const response = await parseResume(token, result.id);
      if (response.status === "ready" && response.data) {
        setProfile(response.data);
      } else {
        setParseError(
          response.error_message ?? "Couldn't extract a profile from this resume."
        );
      }
    } catch (err) {
  ...
  return (
    <div>
      ...
      <div className="mt-4 rounded-2xl border border-line bg-white p-6">
        <label
          htmlFor="resume-file"
          className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-line px-6 py-8 text-center transition-colors hover:border-navy/40"
        >
      ...
      {profile && <CandidateProfileCard data={profile} />}
